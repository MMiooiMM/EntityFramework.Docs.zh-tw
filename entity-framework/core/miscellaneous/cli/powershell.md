...
| 參數                          | 描述                                                                                                                                                                                                                                                             |
|:-----------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <nobr>Test-connection \<String ></nobr> | 資料庫的連接字串。 針對 ASP.NET Core 2.x 專案，此值可以是*name =\<名稱的連接字串 >* 。 在此情況下，此名稱來自針對專案所設定的設定來源。 這是位置參數，而且是必要的。 |
| <nobr>-Provider \<String ></nobr>   | 要使用的提供者。 這通常是 NuGet 套件的名稱，例如： `Microsoft.EntityFrameworkCore.SqlServer`。 這是位置參數，而且是必要的。                                                                                           |
| -OutputDir \<String >               | 要用來放置檔案的目錄。 路徑相對於專案目錄。                                                                                                                                                                                             |
| -CoNtextDir \<String >              | 要放入 `DbContext` 檔案的目錄。 路徑相對於專案目錄。                                                                                                                                                                              |
